<template>
  <div>
    ...
    <div class="mt-4" v-if="topThreeOrigins.length!=0 && isVisible">
      ...
      <ul class="mt-2">
        <li v-for="(origin, index) in topThreeOrigins" :key="index" class="mb-2">
          <span class="font-semibold">{{ origin.name.split(',')[0] }}</span>, Distance: {{ origin.distance.toFixed(2) }} km
        </li>
      </ul>
    </div>
  </div>
...
const searchRoute = async () => {
  ...
  const destinationQuery = encodeURIComponent(`${detectedLocation.value} ${city}`);
  console.log(destinationQuery);

  // Filter out empty origin inputs
const originQueries = responders.value
  .filter(responder => responder.name.trim() !== '' && responder.location === city && responder.type==props.type)
...
const detectLocation = () => {  
  ...
  if (matchedLocation) {
    ...
    detectedLocation.value = maxSimilarityIndex !== -1 ? locations.value[maxSimilarityIndex] : '';
  }
};
...
watch([() => props.inputText, () => props.type], ([newInputText, newType], [oldInputText, oldType]) => {
